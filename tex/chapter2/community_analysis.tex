...
Along with raw sequencing outputs (\texttt{fastq.gz} files) for each sample, Elviz data was provided \cite{cantor2015} by JGI.
...
Again, this used BWA-MEM \cite{li2009}, and a \href{https://github.com/BeckResearchLab/meta4/blob/master/m4b_binning/assembly/data/sample_info/count_reads_in_each_sample.sh}{script} written to count raw \texttt{.fastq} reads.
...
\centering
    ...
    \caption[MyCC binned contigs more aggressively than MetaBAT]{
        ...
        With the settings used, MyCC binned %>% 99.9\% of contigs at all lengths, for all lengths.
...
While the assembly used throughout this chapter was appropriate for that question, there is potential to do different assemblies that target particular organisms of interest.
...
Special care can be taken to isolate different \textit{Methylobacter} strains.
...
Though Lanthanum was not intentionally supplied to the organisms in this study, its possible that it was present in trace amounts.
...
In conclusion, this 88-metagenome, 88-metatranscriptome data set was designed to allow identification of dominant taxa and gene expression in a rich microbial community.
